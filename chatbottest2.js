(function() { // IIFE (Immediately Invoked Function Expression) to prevent global scope pollution
    ...
    const chatbotStyles = `
        ...
        #chatbot-root-container .bot-message ol li {
            display: flex !important; /* Changed from list-item to flex for alignment with custom marker */
            align-items: flex-start !important;
        ...
        #chatbot-root-container .bot-message strong {
            color: #fff !important; /* Changed to white as per request */
        }
    ...
    function _initChatbotElements() {
        ...
        const isNewBrowserSession = !localStorage.getItem('chatbotSessionId'); // Check if a session ID even exists from before

        if (isNewBrowserSession || AUTO_OPEN_CHAT) { // If it's a new session or auto-open is active, reset
