(function() { // IIFE (Immediately Invoked Function Expression) to prevent global scope pollution
    ...
    const chatbotStyles = `
        ...
        #chatbot-root-container #send-button i {
            font-size: 1.2em !important; /* Adjust icon size for FA */
        ...
        #chatbot-root-container .close-button i { /* Styling for Font Awesome close icon */
        ...
        #chatbot-root-container #send-button i { /* Styling for Font Awesome send icon */
            font-size: 1.2em !important;
            line-height: 1 !important;
        }
    ...
    // Dynamic values for icons, logo, and brand name are embedded.
    ...
    function initializeChatbot() {
        ...
        if (typeof marked === 'undefined') {
            ...
            // Wait for both Font Awesome and marked.js to load before initializing elements
            let loadedCount = 0;
            const checkBothLoaded = () => {
                loadedCount++;
                if (loadedCount === 2) _initChatbotElements();
            };
            fontAwesomeLink.onload = checkBothLoaded;
            markedScript.onload = checkBothLoaded;
            document.head.appendChild(markedScript);
        } else {
             // If marked.js is already there, just wait for Font Awesome
             fontAwesomeLink.onload = _initChatbotElements;
        }
    }
    ...
    function _initChatbotElements() {
        ...
        // A new conversation starts if no 'chatbotHasConversation' flag exists OR if auto-open is enabled
        // This ensures suggestions reset when browser is refreshed and a new conversation starts.
        const isNewConversation = !localStorage.getItem('chatbotHasConversation') || AUTO_OPEN_CHAT;
        ...
        function populateSuggestions() {
            ...
            const hasSentMessageInSession = localStorage.getItem('chatbotHasConversation') === 'true';

            if (predefinedSuggestions.length === 0 || (SHOW_SUGGESTIONS_ONCE && hasSentMessageInSession)) {
        ...
        function openChat() {
            ...
            if (isChatBoxEmpty) { 
                ...
                if (!SHOW_SUGGESTIONS_ONCE && predefinedSuggestions.length > 0) {
                    suggestionsArea.classList.remove('hidden-element');
                } else if (SHOW_SUGGESTIONS_ONCE && !hasSentMessageInSession && predefinedSuggestions.length > 0) {
