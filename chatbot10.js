(function() { // IIFE (Immediately Invoked Function Expression) to prevent global scope pollution
    ...
    // Customizable colors (Will override CSS variables in :root)
    const PRIMARY_COLOR = currentScript.dataset.primaryColor || '#6C5CE7'; 
    const SECONDARY_COLOR = currentScript.dataset.secondaryColor || '#5a4acb'; 

    // Customizable Font Awesome icon classes (new data attributes) - IMPORTANT: Ensure these are correct FA 6 classes
    const CHAT_ICON_CLASS = currentScript.dataset.chatIconClass || 'fa-solid fa-comment'; // Default: solid comment icon
    const CLOSE_ICON_CLASS = currentScript.dataset.closeIconClass || 'fa-solid fa-xmark'; // Default: solid x-mark icon
    const SEND_ICON_CLASS = currentScript.dataset.sendIconClass || 'fa-solid fa-paper-plane'; // Default: solid paper plane icon
    ...
    // --- CSS Styles (as a single string) - Based on your provided CSS, modified for embedding ---
    const chatbotStyles = `
        :root {
        ...
        /* Apply !important to every property to ensure it overrides client's CSS */
        #chatbot-root-container *, #chatbot-root-container *::before, #chatbot-root-container *::after {
            ...
            box-shadow: none !important; /* Ensure no box-shadow from client CSS */
            text-shadow: none !important; /* Ensure no text-shadow from client CSS */
            -webkit-text-stroke: 0 !important; /* Ensure no text-stroke from client CSS */
        }
        #chatbot-root-container {
            ...
            width: 0 !important;
            height: 0 !important; /* Take no layout space on client's page */
        }
        ...
        #chatbot-root-container #send-button i {
            ...
            color: inherit !important;
            /* Explicitly set FA font properties for max compatibility */
            font-family: "Font Awesome 6 Free" !important;
            font-weight: 900 !important; /* Most solid FA icons use 900, adjust if you use brands/regular */
            font-style: normal !important; /* Override potential italic from client CSS */
        }
        ...
        #chatbot-root-container .chat-window.hidden-display { /* Using this for hidden instead of 'hidden' */
        ...
        #chatbot-root-container .brand-logo {
            ...
            height: auto !important; /* Set height to auto for proper scaling */
            max-width: 100% !important; /* Ensure it doesn't overflow its wrapper */
            border-radius: 50% !important;
        ...
        /* Reset for all child elements within bot-message to ensure consistent rendering */
        #chatbot-root-container .bot-message * {
            margin: 0 !important;
            padding: 0 !important;
        ...
        #chatbot-root-container .bot-message ol {
            ...
            margin-left: 20px !important; /* Keep indentation for the list itself */
            margin-bottom: 8px !important;
        }
        #chatbot-root-container .bot-message ul li,
        #chatbot-root-container .bot-message ol li {
            display: flex !important; /* IMPORTANT: Use flex for list items for proper alignment of marker and text */
            align-items: flex-start !important; /* Align marker and text to the top */
            margin-bottom: 5px !important;
            line-height: 1.4 !important;
        }
        /* Specific styling for <p> tags directly inside <li> elements */
        #chatbot-root-container .bot-message ul li p,
        #chatbot-root-container .bot-message ol li p {
            flex-grow: 1 !important; /* Allow paragraph to take up remaining space in flex container */
            margin-bottom: 0 !important; /* Remove bottom margin for p inside li */
            padding-right: 5px !important; /* Small padding for visual separation if needed */
            min-width: 0 !important; /* Allow text to wrap within flex item */
        ...
        #chatbot-root-container .bot-message ol li::before {
            ...
            display: inline-block !important; /* Treat marker as an inline block for alignment */
            vertical-align: top !important; /* Align marker to the top of the line box */
        }
    ...
    // --- HTML Structure (as a single string) - Based on your provided HTML ---
    const chatbotHTML = `
    ...
    function initializeChatbot() {
        ...
        setTimeout(() => { // Small delay to allow browser to parse stylesheet if very fast
            if (!fontAwesomeLoaded && fontAwesomeLink.sheet && fontAwesomeLink.sheet.cssRules.length > 0) {
    ...
    function _initChatbotElements() {
        ...
        if (isNewBrowserSession || (AUTO_OPEN_CHAT && !hasConversationStartedPreviously)) { 
